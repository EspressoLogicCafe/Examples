(function () {
	...
	var database = settings && settings.database;
    ...
    out.println("catalog:"+catalogName+",schemaName:"+schemaName);
	...
	function createAttrType(type, typeName, size, length, precision, scale) {
	    out.println("type:"+ type +" , typeName:" + typeName);
		var attrTypeName;
		switch (type) {
		default:
		...
		out.println("smallint "+ typeName);
		    if(typeName.indexOf("UNSIGNED") > 0) {
		        attrTypeName = 'SMALLINT UNSIGNED';
    			return dspFactory.createUnsignedInteger(attrTypeName, 2);
		    } 
		    else {
		...
		case 4: // java.sql.Types.INTEGER;
		    var prefix = 3 === precision ? "MEDIUM ": "INT";
		    if(typeName.indexOf("UNSIGNED") > 0) {
		        attrTypeName = prefix + ' UNSIGNED';
    			return dspFactory.createUnsignedInteger(attrTypeName, 4);
		    } 
		    else {
    			attrTypeName = prefix;
    			return dspFactory.createInteger(attrTypeName, 4);
		    }
            break;
		case -5: // java.sql.Types.BIGINT
		    if(typeName.indexOf("UNSIGNED") > 0) {
		        attrTypeName = 'BIGINT UNSIGNED';
    			return dspFactory.createUnsignedInteger(attrTypeName, 8);
		    } 
		    else {
		    	attrTypeName = 'BIGINT';
        ...
        case -6: // java.sql.Types.TINYINT
		    if(typeName.indexOf("UNSIGNED") > 0) {
		        attrTypeName = 'TINYINT UNSIGNED';
    			return dspFactory.createUnsignedInteger(attrTypeName, 1);
		    } 
		    else {
		    	attrTypeName = 'TINYINT';
		    	return dspFactory.createTinyInt(attrTypeName);
		    }
        ...
        case -7: //  java.sql.Types.Bit
            if(1 === precision) {
                ...
                return dspFactory.createBitmask("bit("+ precision +")", precision);
            }
           
		case 93: // java.sql.Types.TIMESTAMP;
			attrTypeName = null !== typeName? typeName: 'TIMESTAMP';
			if("DATETIME" === attrTypeName) {
			    return dspFactory.createTimestamp(attrTypeName, 9);
			} else {
			    return dspFactory.createTimestampWithZone(attrTypeName, 9);
			}
            break;
		case 92: // java.sql.Types.TIME
			attrTypeName = 'TIME';
			return dspFactory.createIntervalTime(attrTypeName);
		}
	}
	...
	var tbls = databaseMetaData.getTables(catalogName, database, '%', type);
	var key;
	...
	while (proceduresRS.next()) {
        ...
        out.println("procedureName: "+procedureName );
		var proc = {
			name: procedureName,
			isFunction: false,
	...
	while (functionsRS.next()) {
		...
		if(null !== structure.procedures.get(functionName)) {
		    continue;
		}
		...
		structure.procedures.add(funct);
	}
	functionsRS.close();
	...
	for (var i = 0; i < structure.procedures.length; ++i) {
		...
		out.println('processing parameters for ' + proc.name);
		...
		while (procColumnsRS.next()) {
			...
			out.println('Argument ' + procParameterName);
